...
export async function main(req) {
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      const lineHeight = 10;
      const scale =
        e.deltaMode == 1 ? lineHeight : e.deltaMode == 2 ? height : 1;
      let inv = invert4(viewMatrix);
      if (e.shiftKey) {
        inv = translate4(
          inv,
          (e.deltaX * scale) / width,
          (e.deltaY * scale) / height,
          0
        );
        ...
        // let preY = inv[13];
        inv = translate4(inv, 0, 0, (-10 * (e.deltaY * scale)) / height);
        // inv[13] = preY;
      } else {
        let d = 4;
        inv = translate4(inv, 0, 0, d);
        inv = rotate4(inv, -(e.deltaX * scale) / width, 0, 1, 0);
        inv = rotate4(inv, (e.deltaY * scale) / height, 1, 0, 0);
        inv = translate4(inv, 0, 0, -d);
      }
  ...
  canvas.addEventListener("mousemove", (e) => {
    ...
    if (down == 1) {
      let inv = invert4(viewMatrix);
      let dx = (5 * (e.clientX - startX)) / width;
      let dy = (5 * (e.clientY - startY)) / height;
      let d = 4;
      ...
      inv = translate4(
        inv,
        (-10 * (e.clientX - startX)) / width,
        0,
        (10 * (e.clientY - startY)) / height
      );
      // inv[13] = preY;
      viewMatrix = invert4(inv);
  ...
  canvas.addEventListener(
    ...
    (e) => {
      ...
      if (e.touches.length === 1 && down) {
        let inv = invert4(viewMatrix);
        let dx = (4 * (e.touches[0].clientX - startX)) / width;
        let dy = (4 * (e.touches[0].clientY - startY)) / height;
        ...
        inv = translate4(inv, -dx / width, -dy / height, 0);
  ...
  // while (true) {
  //   if (bytesRead >= splatData.length || stopLoading) break;
  ...
  //   });
  // }
  // main 함수가 반환될 때 cleanup 설정
  return () => {
    isMounted = false; // 루프 중단
  };
}
