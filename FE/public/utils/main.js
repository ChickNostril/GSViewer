...
async function main() {
  ...
  try {
    ...
    if (!req.ok) {
      throw new Error(`FastAPI 호출 실패: ${req.status} - ${req.statusText}`);
    }

    // 바이너리 데이터 로드
    const responseBuffer = await req.arrayBuffer();
    splatData = new Uint8Array(responseBuffer);
    console.log("FastAPI에서 데이터 로드 완료:", splatData);
  } catch (error) {
    console.warn("FastAPI 호출 실패. 기본 URL에서 데이터를 로드합니다:", error);

    // FastAPI 실패 시 기본 URL로 전환
    const url = new URL(
      params.get("url") || "train.splat",
      "https://huggingface.co/cakewalk/splat-data/resolve/main/"
    );
    ...
    if (!req.ok) {
      throw new Error(`기본 URL 호출 실패: ${req.status} - ${req.statusText}`);
    }

    // 바이너리 데이터 로드
    const responseBuffer = await req.arrayBuffer();
    splatData = new Uint8Array(responseBuffer);
    console.log("기본 URL에서 데이터 로드 완료:", splatData);
  }

  // 데이터 로드 후 WebGL 초기화 및 Worker 처리
  const rowLength = 3 * 4 + 3 * 4 + 4 + 4;
  const downsample =
...
main().catch((err) => {
  ...
  if (message) {
    ...
    console.error("Error message container not found:", err);
  }
});
